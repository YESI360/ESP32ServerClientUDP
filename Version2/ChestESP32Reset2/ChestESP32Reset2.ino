...
String calMessage = "CC, ";//VALOR CALIBRADO CHEST
...
////////////////////TIEMPO EN MILIS QUE TOMA PARA HACER LA CALIBRACION//////////////////
  ...
  initRutina = millis() + 30000; //calCITY (version calibracion pd) [reset en 15]
}
...
void step() {
  ...
  if (millis() > initRutina)/////////////////envia valores ya calibrados. CAMBIOS DE ESTADO 1 Y 2
  {
    ...
    else if ( (millis() - elViejoTiempo) < intervalleEntreResp / 4) {// si estamos dentro de un tercio del intervalo de respiración. el 3 es un numero que daba mejor...
      digitalWrite (LED_BUILTIN, LOW);
      Udp.beginPacket(ip, 8888);
      Udp.print("chest, 2");
      Udp.endPacket();
    ...
    if (packetSize) {
      ...
      */
      ...
      if (packetBuffer[0] == 'c') { // This compares the received message (we're sending '1')
        //"Resetting";
        invertScreen == true;
        initRutina = millis () + 10000;//5000;
        //ESP.restart();
      }
    }
} // fin step
